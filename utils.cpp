...
// Decryption of telegram in AES CTR mode
bool decrypt_TPL_AES_CTR_IV(std::vector<unsigned char> &frame,
                            std::vector<unsigned char>::iterator &pos,
                            std::vector<unsigned char> &key,
                            unsigned char *iv,
                            int *num_encrypted_bytes,
  ...
  *num_not_encrypted_at_end = buffer.size()-num_bytes_to_decrypt;
  ...
  AES_CTR_xcrypt_buffer(&ctx, decrypted_data, num_bytes_to_decrypt);
